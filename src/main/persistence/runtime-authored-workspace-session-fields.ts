 ...
 *
 * `setWorkspaceSession` replaces the stored object, and the renderer builds its payload from
 * Zustand -- which has no idea the runtime authority sharing this profile also persists the
 * client-hosted pages it owns. Without this, every ordinary desktop session write erases them, and
 * the loss only shows up a restart later when there is nothing left to rehydrate.
 *
